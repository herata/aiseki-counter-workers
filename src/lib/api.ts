...
const API_BASE_URL = process.env.API_BASE_URL || 'https://5ikj3hpnz27cdwpbz2ndrokvyi0aazgl.lambda-url.ap-northeast-1.on.aws';
  ...
  getVisitorData: async (request: VisitorDataRequest): Promise<VisitorData> => {
    ...
    return {
      ...
      data: rawData.data.map(item => ({
        ...
        total: Number.parseInt(item.male, 10) + Number.parseInt(item.female, 10),
      })),
    };
  },
};
